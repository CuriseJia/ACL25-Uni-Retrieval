...
pair = json.load(open('imagenet/200-original-long.json', 'r'))
other = json.load(open('imagenet/200-original-sketch-0-10.json', 'r'))
acc = []
for i in tqdm(range(0, 10)):
    for j in range(64):
        index = random.randint(0, len(pair))
        # text_list.append(pair[index]['caption'])
        ori_image.append(osp.join('imagenet/imagenet-p/train/', pair[index]['image_path']))
